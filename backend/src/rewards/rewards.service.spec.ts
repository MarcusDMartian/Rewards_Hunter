...
describe('RewardsService', () => {
  ...
  describe('redeem', () => {
    it('should throw NotFoundException for bad user', async () => {
      mockPrisma.user.findUnique.mockResolvedValue(null);
      await expect(service.redeem('bad', 'r1')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw NotFoundException for bad reward', async () => {
      ...
      await expect(service.redeem('u1', 'bad')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw BadRequestException if out of stock', async () => {
      ...
      await expect(service.redeem('u1', 'r1')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('processRedemption', () => {
    it('should throw NotFoundException for bad redemption', async () => {
      ...
      await expect(
        service.processRedemption('bad', 'Approved', 'admin'),
