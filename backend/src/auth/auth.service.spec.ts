...
describe('AuthService', () => {
    ...
    describe('checkDomain', () => {
        ...
        it('should report userExists when user already exists', async () => {
            mockPrisma.organization.findUnique.mockResolvedValue({ id: 'org1', domain: 'known.com' });
            mockPrisma.user.findUnique.mockResolvedValue({ id: 'u1', email: 'test@known.com' });
    ...
    describe('login', () => {
        ...
        it('should throw UnauthorizedException for inactive user', async () => {
            ...
            await expect(service.login({ email: 'test@test.com', password: 'correct-password' }))
                .rejects.toThrow(UnauthorizedException);
        });
    });
    ...
    describe('registerOrg', () => {
        it('should throw ConflictException if domain exists', async () => {
            ...
            await expect(service.registerOrg({
        ...
        it('should create org and return token on success', async () => {
            ...
            mockPrisma.$transaction.mockImplementation(async (cb) => {
                return cb({
                    organization: { create: jest.fn().mockResolvedValue({ id: 'org1', name: 'New Org', domain: 'new.com' }) },
                    team: { create: jest.fn().mockResolvedValue({ id: 't1', name: 'General' }) },
                    user: {
                        create: jest.fn().mockResolvedValue({
                            id: 'u1', email: 'admin@new.com', name: 'Admin', role: 'Superadmin', passwordHash: 'hash',
                        })
                    },
                });
            });
    ...
    describe('submitJoinRequest', () => {
        it('should throw NotFoundException if org not found', async () => {
            mockPrisma.organization.findUnique.mockResolvedValue(null);
            await expect(service.submitJoinRequest({
                email: 'user@test.com', password: '123456', name: 'User', orgId: 'bad-org',
        ...
        it('should throw ConflictException if user exists', async () => {
            ...
            mockPrisma.user.findUnique.mockResolvedValue({ id: 'u1' });
            await expect(service.submitJoinRequest({
                email: 'user@test.com', password: '123456', name: 'User', orgId: 'org1',
            })).rejects.toThrow(ConflictException);
        });
    ...
    describe('approveJoinRequest', () => {
        it('should throw NotFoundException if request not found', async () => {
            ...
            await expect(service.approveJoinRequest('bad-id'))
                .rejects.toThrow(NotFoundException);
        });

        it('should approve and create user', async () => {
            mockPrisma.joinRequest.findUnique.mockResolvedValue({
                id: 'jr1', email: 'user@test.com', name: 'User', passwordHash: 'h', orgId: 'org1',
            });
            ...
            mockPrisma.$transaction.mockImplementation(async (cb) => {
                return cb({
                    ...
                    user: {
                        create: jest.fn().mockResolvedValue({
                            id: 'u1', email: 'user@test.com', name: 'User', role: 'Member', passwordHash: 'h',
                        })
                    },
                });
            });
    ...
    describe('rejectJoinRequest', () => {
        it('should throw NotFoundException if request not found', async () => {
            mockPrisma.joinRequest.findUnique.mockResolvedValue(null);
            await expect(service.rejectJoinRequest('bad-id'))
                .rejects.toThrow(NotFoundException);
        });

        it('should reject successfully', async () => {
            mockPrisma.joinRequest.findUnique.mockResolvedValue({ id: 'jr1' });
            mockPrisma.joinRequest.update.mockResolvedValue({ id: 'jr1', status: 'REJECTED' });
    ...
    describe('getUserById', () => {
        ...
        it('should return formatted user without passwordHash', async () => {
            mockPrisma.user.findUnique.mockResolvedValue({
                id: 'u1', email: 'test@test.com', name: 'Test', passwordHash: 'secret',
                role: 'Member', userBadges: [], team: { name: 'Engineering' },
            });
